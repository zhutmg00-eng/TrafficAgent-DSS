...
- 证据：`src/web/app.py:84` 为 `APP_VERSION = "2.3.0"`，当前 HEAD/README/CHANGELOG 已到 v2.3.2。
- 影响：`/api/status` 和 OpenAPI 文档会报告旧版本，排障、演示和发布追踪困难。
- 建议：以单一版本来源生成包版本、API 版本、前端显示和 CHANGELOG 校验，CI 中增加一致性检查。

#### P2-2：技术方案仍有 OSM、HBEFA、方差和 Reflexion 口径漂移
...
- 影响：同一项目的技术方案、README 和代码对数据来源与算法能力的描述不一致，答辩材料容易被追问“实现在哪里”。
- 建议：以当前代码和可复现实验为唯一口径，明确 OSM 仅用于地图/中观网络、SUMO 使用标定走廊；删除未实现 Reflexion 的已实现措辞，改正排放与方差定义。
...
- 影响：缺失数据和真实零排放/零流量无法区分，报告或图表可能把“未测得”误读成“零”。
- 建议：缺失总量返回 `None` 并携带缺失字段；只有明确采样到零值时才返回 0；前端对缺失状态显示“无数据”。
